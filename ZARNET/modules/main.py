...
╚══════╝╚═╝  ╚═╝╚═╝  ╚═╝""")
	...
	def _add_commands(self):
		...
		event.help(["exit", "quit", "e", "q"], "Exit ZAR.")
		event.help(".", "Run a shell command.")
		event.help("clear", "Clear the screen.")
		event.help("ddos", "Connect to a ZAR server.")
		event.help_comment("\nModules:")
	...
	def upgrade():
		try:
			chdir("/tmp")
			system("sudo git clone ttps://github.com/Taguar258/aven-Strm.git")
			...
			print("Cound not upgrade ZAR. (Make sure to use sudo)")
